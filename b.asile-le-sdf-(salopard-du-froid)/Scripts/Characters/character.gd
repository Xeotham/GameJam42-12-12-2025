...
var anim_map : Dictionary = {
	...
	States.SPECIAL:		"Special"
}

var attack_combo_index := 0
var killed_by: DamageReceiver.HitType = DamageReceiver.HitType.NONE
...
func handle_death(delta: float) -> void:
	if state == States.DEATH and not can_respawn:
		...
		if killed_by == DamageReceiver.HitType.SPECIAL:
			velocity = Vector2.ZERO
			character_sprite.modulate = Color(0, 0.5, 1)
		elif killed_by != DamageReceiver.HitType.NONE:
			velocity = Vector2.ZERO
			character_sprite.modulate = Color(0.8, 0.2, 0.2)
		if modulate.a <= 0:
			queue_free()
...
func can_get_hurt() -> bool:
	return [States.IDLE, States.WALK].has(state)
...
func on_emit_damage(receiver: DamageReceiver) -> void:
	...
	if state == States.SPECIAL:
		hit_type = DamageReceiver.HitType.SPECIAL
